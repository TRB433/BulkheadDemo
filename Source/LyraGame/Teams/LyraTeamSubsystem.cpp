...
ULyraTeamSubsystem::ULyraTeamSubsystem() : teamDamage(false) //by default team damage is off
{
}
...
bool ULyraTeamSubsystem::CanCauseDamage(const UObject* Instigator, const UObject* Target, bool bAllowDamageToSelf) const
{
	...
	//can cause damage to player if they are on different teams or team damage is on
	if (Relationship == ELyraTeamComparison::DifferentTeams || teamDamage) 
	{				
		return true;		
	}
...
//returns true if friendly player is hit
//could be used to punish player for hitting friendlies a certain amount of times
//if team damage isn't enabled, will never return true
//bool ULyraTeamSubsystem::hitFriendly(const UObject* Instigator, const UObject* Target, bool bAllowDamageToSelf) const {
//	int32 InstigatorTeamId;
//	int32 TargetTeamId;

	//getting relationship between instigator and target
//	const ELyraTeamComparison Relationship = CompareTeams(Instigator, Target, /*out*/ InstigatorTeamId, /*out*/ TargetTeamId);	
//	if (Relationship == ELyraTeamComparison::OnSameTeam && teamDamage) 
//	{
//		return true;
//	}
//}
